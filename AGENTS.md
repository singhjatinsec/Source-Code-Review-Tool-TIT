...
5. One issue at a time. If the issue is larger than its size label suggests, split it: create follow-up issues with the same structure and link them.
...
- Never commit secrets, real credentials, client code, vault files or ledger files. Test fixtures use obviously fake values that still match detector patterns.
